...
BigEndian();
// Determine the founds of the MasterTrack info
local int64 globalOffset = FindFirst("GDocument", true, true, 0, 0.0, 1, 0, 0, 24) - 8;
...
Printf("\nGlobal Offset = %Ld", globalOffset);
Printf("\nMTempoTrackEvent Start = %Ld", MTTE_pos);
Printf("\nMTempoTrackEvent Bound = %Ld", MTTE_bound);
Printf("\nMSignatureTrackEvent Start = %Ld", MSTE_pos);
Printf("\nMSignatureTrackEvent Bound = %Ld", MSTE_bound);
...
// Define the headers
BigEndian();
char beginning_of_file[MTTE_pos] <fgcolor=cDkGray, bgcolor=cBlack>;
sectionType = 0;
